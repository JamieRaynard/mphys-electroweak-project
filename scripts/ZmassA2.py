...
with uproot.open(f"{DATAIR}/DecayTree__Z__Z__d13600GeV_24c4.root:DecayTree") as t:
    ...
    datam=t.arrays(["mum_eta","mum_phi","mum_pt","mup_eta" ,"mup_phi" ,"mup_pt","true_boson_mass"],library="np")
...
def plot(tmass,datam):
    massHist,binn,_=plt.hist(conaeq(datam), bins=100, histtype="step",range=(80,100),label="Z-reconstructed",density=True,linewidth=1) #for reconstructed mass
    ...
    plt.title("Z-reconstucted and fit")
    plt.xlabel("Mass_Gev")
    plt.ylabel("Frequency Density")
    plt.legend(loc='upper right')
    plt.savefig("Z-reconstucted and fit.pdf")
    plt.clf()

    plt.figure()  #now making a grpah with a histogram with woeghtings applied
    ratio=fiteq(91.188,fitParam[0],fitParam[1],fitParam[2],fitParam[3],fitParam[4])/fiteq(tmass,fitParam[0],fitParam[1],fitParam[2],fitParam[3],fitParam[4])
    massHistweights,binnweights,_weights=plt.hist(conaeq(datam), bins=100, histtype="step",range=(80,100),weights=ratio,label="Z-reconstructed",density=True,linewidth=1) #for reconstructed mass with wieghts
    plt.plot(centers,model)
    plt.title("Z-reconstucted weighted")
    plt.xlabel("Mass_Gev")
    plt.ylabel("Frequency Density")
    plt.legend(loc='upper right')
